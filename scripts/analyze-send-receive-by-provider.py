#!/usr/bin/env python3
"""
Analyze Cannot Send/Receive Emails issues by email provider.

This script breaks down send/receive pain points by email provider.
"""
...
def load_send_receive_question_ids(csv_path):
    """Load Cannot Send/Receive question IDs from pain point CSV."""
    with open(csv_path, 'r', encoding='utf-8') as f:
        reader = csv.DictReader(f)
        for row in reader:
            if row.get('pain_point') == 'Cannot Send/Receive Emails':
...
def identify_email_provider(question):
    ...
    providers = [
        ...
        {
            'name': 'ionos',
            'display_name': 'IONOS (1&1)',
            'patterns': ['ionos', '1&1', '1and1']
        },
...
def categorize_by_provider(questions, send_receive_ids):
    """Categorize send/receive questions by email provider."""
    provider_questions = defaultdict(list)
    provider_display_names = {}

    for qid in send_receive_ids:
...
def write_markdown_report(md_path, provider_questions, provider_display_names, total_send_receive, year, month, product):
    ...
    with open(md_path, 'w', encoding='utf-8') as f:
        f.write(f'# {product_name} Cannot Send/Receive Emails by Email Provider - {year:04d}-{month:02d}\n\n')
        f.write(f'**Total Cannot Send/Receive Issues:** {total_send_receive}\n\n')
        ...
        for provider_name, questions in sorted_providers:
            display_name = provider_display_names[provider_name]
            count = len(questions)
            percentage = count * 100.0 / total_send_receive
...
def main():
    if len(sys.argv) == 4:
        ...
        # Default to March 2026 desktop
        product = 'desktop'
        year = 2026
        month = 3

    print(f"Analyzing Cannot Send/Receive issues by provider for {product} {year:04d}-{month:02d}")
    ...
    # Load Cannot Send/Receive question IDs
    pain_point_csv = Path('REPORTS') / product.upper() / f'{year:04d}-{month:02d}-{product}-top-pain-points.csv'
    print(f"Loading Cannot Send/Receive IDs from {pain_point_csv}")
    send_receive_ids = load_send_receive_question_ids(pain_point_csv)
    print(f"  Found {len(send_receive_ids)} Cannot Send/Receive questions")

    # Categorize by provider
    print("Categorizing by email provider...")
    provider_questions, provider_display_names = categorize_by_provider(questions, send_receive_ids)
    ...
    for provider_name, questions in sorted_providers:
        display_name = provider_display_names[provider_name]
        count = len(questions)
        percentage = count * 100.0 / len(send_receive_ids)
    ...
    # Write CSV report
    csv_filename = f"{year:04d}-{month:02d}-{product}-send-receive-by-provider.csv"
    ...
    md_filename = f"{year:04d}-{month:02d}-{product}-send-receive-by-provider.md"
    md_path = reports_dir / md_filename
    write_markdown_report(md_path, provider_questions, provider_display_names, len(send_receive_ids), year, month, product)
