#!/usr/bin/env python3
"""
Compare Cannot Send/Receive Emails issues by email provider across two months.
...
def write_markdown_report(md_path, comparisons, month1_name, month2_name, month1_total, month2_total, product):
    ...
    with open(md_path, 'w', encoding='utf-8') as f:
        f.write(f'# {product_name} Cannot Send/Receive Emails by Email Provider - {month1_name} vs {month2_name}\n\n')
        ...
        f.write(f'- **{month1_name} Total:** {month1_total} Cannot Send/Receive issues\n')
        f.write(f'- **{month2_name} Total:** {month2_total} Cannot Send/Receive issues\n')
...
def main():
    if len(sys.argv) == 5:
        ...
        # Default: Compare Feb 2026 vs Mar 2026 for desktop
        product = 'desktop'
        year = 2026
        month1 = 2
        month2 = 3
    ...
    print(f"Comparing Cannot Send/Receive provider data: {month1_name} vs {month2_name}")

    # Load provider data
    reports_dir = Path('REPORTS') / product.upper()

    month1_csv = reports_dir / f'{month1_name}-{product}-send-receive-by-provider.csv'
    month2_csv = reports_dir / f'{month2_name}-{product}-send-receive-by-provider.csv'
    ...
    print(f"  Total: {month1_total} Cannot Send/Receive issues")
    ...
    print(f"  Total: {month2_total} Cannot Send/Receive issues")
    ...
    # Write reports
    csv_filename = f'{month1_name}_vs_{month2_name}-{product}-send-receive-provider-comparison.csv'
    csv_path = reports_dir / csv_filename
    write_csv_report(csv_path, comparisons, month1_name, month2_name)
    print(f"\nWrote CSV report: {csv_path}")

    md_filename = f'{month1_name}_vs_{month2_name}-{product}-send-receive-provider-comparison.md'
