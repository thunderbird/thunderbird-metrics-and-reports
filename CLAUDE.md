...
- **CRUCIAL LOCKED DECISION — always link a SUMO question.** Every question id
  ...
  it, with date, linked id, version, 80-character title, answered, solved (the
  SUMO `is_solved` flag) and whether the most recent answer came from somebody
  on the trusted-contributors list (see
  `PROJECT1/validation/incident-spectrum-2026-08.md`). A person is trusted only
  when they are on that CSV; a helper with a handful of answers is correctly
  "no", which is not a staleness artefact of the list.
