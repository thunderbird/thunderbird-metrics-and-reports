---
layout: base
title: "2026-08 exec summary in plain English: Thunderbird Desktop support spikes"
---

# August 2026 in plain English: Thunderbird Desktop support spikes

This page is a plain-English rewrite of the [August 2026 executive summary](2026-08-exec-summary.html).
The numbers are the same. Both pages come from the same 941 Thunderbird Desktop
support questions of August 2026. No AI read the questions. The tool uses regular
expressions and standard statistics only. The source page carried the timestamp
2026-09-10 00:19 UTC.
...
## Result for August 2026

The tool found 27 spikes worth a look in August 2026. 17 of them tie to a
Thunderbird version, and 10 are cause-level.
...
- Volume: 941 questions. 419 of them (45%) carry a cause tag. The count per day
  was `▄▃▅▅▅▅▆▆▃▅▃▅▅▅▄▅▅▆▄▅▆▅▄▆▅█▆▅▄▄▇`, one block per day from August 1 to August 31.
...
- Release-adoption version spikes: 31. Users moved to Thunderbird 154 and then 155 during the month, so the bare counts rose. These are not incidents.

Unlike the older months, August 2026 carries the Thunderbird version for most
questions, so the version×cause detector works here.

## What the spikes say

Printing broke in Thunderbird 154. The pair `v154 × feat:printing` fired seven
times, more than any other pair in the month. It reached 29 questions for the
month at 10.5 times the expected count. The first day it fired was August 20,
four days after the release, at 24.3 times the expected count. Counted without
the version, printing reached 36 questions in August against a baseline of 4.5.

Attachment questions rose with the printing questions, and both point at the
same defect. Users write that Thunderbird prints an attached PDF file as a blank
page, so the same question carries the printing tag and the attachment tag. Read
`v154 × feat:attachments` (5 questions, 3.8×) as part of the printing incident,
not as a second one.

The mail host Spectrum was the other cluster. Spectrum reached 34 questions for
the month, more than three times its baseline of 10.5. In the week of August 17
it reached 11 times its baseline. It fired against Thunderbird 153 and 154 alike, so the
problem sits at the mail host and not in one release. A separate page holds the
[Spectrum incident analysis](https://github.com/thunderbird/thunderbird-metrics-and-reports/blob/main/PROJECT1/validation/incident-spectrum-2026-08.md).

Three smaller clusters are in the tables. The mail protocol POP fired against
Thunderbird 153 on August 10, 13 and 14, at four questions per day. Yahoo Mail
reached 19 questions in the week of August 10, three and a half times its
baseline. Microsoft mail fired once against Thunderbird 153, on August 4.

Two clusters were served badly. In the week of August 17, only 43% of the
`v154 × feat:printing` questions and 38% of the printing questions got an
answer. Every other cluster in the month reached 63% or more.
...
therefore move the expected count for a past month. Rows can cross the threshold
in both directions, and the answered percentage rises as late answers land. This
page is a snapshot written by hand on 2026-09-09. The original page regenerates
every day, so it can hold different numbers. Read the two pages together.
...
<details markdown="1">
<summary>Cause-level spikes, 10 rows</summary>
...
Version and operating system are filters, not causes. A rise in the bare count of
one version is release adoption, not a regression. The rows are here for manual
checking only.
...
<details markdown="1">
<summary>August 2026 trends</summary>
...
counts toward August 2026 when its week overlaps the month. Version×cause needs a
